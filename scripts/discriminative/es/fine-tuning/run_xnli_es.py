...
""" Finetuning multi-lingual models on XNLI (e.g. Bert, DistilBERT, XLM), evaluation on XNLIvar.
...
# import wandb
# wandb.login(key="XXX)
# wandb.init(project="xnli-es-zero-shot-finetuning")


parser = argparse.ArgumentParser()
parser.add_argument("--ev_dataset", type=str, default="es", help="")
parser.add_argument("--pred_dataset", type=str, default="es", help="")
...
def main():
    ...
    if training_args.do_predict:
        if model_args.test_data == "es": 
            ...
            predict_dataset = load_dataset(
                'csv',
                data_files="/tartalo01/users/jbengoetxea004/phd/xnli-paraphrasing/data/es/eu-native-in-spanish-with-labels.tsv",
                delimiter="\t",  
                split="train",
                cache_dir=model_args.cache_dir,
                token=model_args.token,
            ...
            predict_dataset = load_dataset(
                'csv',
                data_files="/tartalo01/users/jbengoetxea004/phd/xnli-paraphrasing/data/es/xnli-native-var-es-ALL.tsv",
                delimiter="\t",  
                split="train",
                cache_dir=model_args.cache_dir,
                token=model_args.token,
            ...
            predict_dataset = load_dataset(
                'csv',
                data_files="/tartalo01/users/jbengoetxea004/phd/xnli-paraphrasing/data/en/es-native-in-english-with-labels.tsv",
                delimiter="\t",  
                split="train",
                cache_dir=model_args.cache_dir,
                token=model_args.token,
            ...
            predict_dataset = load_dataset(
                'csv',
                data_files="/tartalo01/users/jbengoetxea004/phd/xnli-paraphrasing/data/en/es-var-in-english-with-labels.tsv",
                delimiter="\t",  
                split="train",
                cache_dir=model_args.cache_dir,
                token=model_args.token,
    ...
    if training_args.do_predict:
        ...
        if model_args.test_data in no_variation: 
            ...
            if trainer.is_world_process_zero():
                with open(output_predict_file, "w") as writer:
                    writer.write("index\tann_id\tchange_type\therrialdea\tprem_id\tpremise\thypothesis\tlabel\tprediction\n")  # Write header
                    for index, (ann_id, change_type, herrialdea, prem_id, premise, hypothesis, label, prediction_idx) in enumerate(zip(
                        predict_dataset["ann_id"], predict_dataset["change_type"], predict_dataset["herrialdea"], predict_dataset["prem_id"], predict_dataset["premise"], predict_dataset["hypothesis"], predict_dataset["label"], predictions
                    )):
                        predicted_label = label_list[prediction_idx]
                        writer.write(f"{index}\t{ann_id}\t{change_type}\t{herrialdea}\t{prem_id}\t{premise}\t{hypothesis}\t{label}\t{predicted_label}\n")
